...
def map_initialization(nodes_dict, edges_dict):  # function to initialise mapf
    ...
    for i in range(0, 360):  # transform vehicle images in every possible direction
        # Create plane images
        ...
        # Create tug images
        tug_piclist.append(pg.transform.rotozoom(tug_pic, i, (1. / 20.)))  # 1/20 is used for scaling the tug image (make tug smaller)
...
def map_running(map_properties, current_states, t):  # function to update the map
    ...
    INPUT:
        ...
        - current_states = dict with id, heading, xy_pos, and vehicle_type of all active vehicles.
        - t = time.
    RETURNS:
    ...
    #draw vehicles (tugs and aircraft) based on has_flight status
    if disp_vehicles:
        for vehicle in current_states.keys():
            ...
            # Determine if this is a tug or aircraft based on has_flight status
    ...
    if disp_aircaft_id:  # if the vehicle id has to be displayed
        for vehicle in current_states.keys():
            # Determine label based on vehicle type
            label = 'TUG: ' if not current_states[vehicle].get("has_flight", False) else 'AC: '
            id_string = label + str(current_states[vehicle]["ac_id"])  # create string with ID
            
            # Different colors for tugs and aircraft
            if not current_states[vehicle].get("has_flight", False):
                col = orange  # Orange for tugs
            else:
                col = red  # Red for aircraft
        ...
        for ac2 in current_states:
            if ac1 != ac2 and current_states[ac1]["xy_pos"] == current_states[ac2]["xy_pos"]:
                ...
                print("COLLISION - between", current_states[ac1]["ac_id"], "and", current_states[ac2]["ac_id"], 
                      "at location", current_states[ac1]["xy_pos"], "time", time)
